...
namespace Lima {
	...
	LimaModel::LimaModel(LimaDevice &device, const std::vector<Vertex> &vertices) : m_LimaDevice{ device } {
	...
	void LimaModel::createVertexBuffers(const std::vector<Vertex> &vertices) {
		...
		void* data;
		vkMapMemory(m_LimaDevice.device(), vertexBufferMemory, 0, bufferSize, 0, &data);
		memcpy(data, vertices.data(), static_cast<size_t>(bufferSize));
		vkUnmapMemory(m_LimaDevice.device(), vertexBufferMemory);
	}
